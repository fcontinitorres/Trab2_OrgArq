...
int tamanhoRegistro(Registro* reg){
	return ( (SIZE_CNPJ*2)+(SIZE_DATA*2)+strlen(reg->razSoc)+strlen(reg->nomeFant)+strlen(reg->motCanc)+strlen(reg->nomeEmp) );
}
...
long int _inserirFF_dado(FILE* file, Registro* reg){
    ...
    if (fragInt != -1) {
        ...
        // encontra o Byte Of set para atualização do anterior 
        if (ant == -1)
...
int insereReg(FILE* file, INDICE* indice, Registro* reg, int op){
	...
	int size = 0;
	int pos = -1; //guarda a posicao inicial de um registro removido
